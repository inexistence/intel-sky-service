...
| Distributed managed-service state change | `processIdentifier,computerUseActive,computerHistoryState` | `com.openai.codex.computer-use.status-item-state-changed`; ChatGPT accepts its cached managed PID and refreshes managed-service state | Authenticated PIP bootstrap starts a socketless exit watchdog; unexpected exit publishes `false/stopped` and ChatGPT respawns/reconnects | Intel `sky.node` strings/metadata + production ASAR + Intel automatic-restart runtime |
